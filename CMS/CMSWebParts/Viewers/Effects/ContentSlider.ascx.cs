...
public partial class CMSWebParts_Viewers_Effects_ContentSlider : CMSAbstractWebPart
{
        ...
        else
        {
            if (!StopProcessing)
            {
                ...
                if (index > 0)
                {
                    ...
                    bottomDiv += "<div id='pagerWrap' class='container'> <div class='device-spacing'> <div id=\"" + ClientID + "_pager\" class=\"Pager\" style=\"width:" + DivWidth + "px;\">";
                    // Page numbers
                    for (int p = 0; p < index; p++)
                    {
                        bottomDiv += "<a class='PagerLink' id=\"" + ClientID + "_page_" + p + "\" href=\"#\" onclick=\"Slider_" + ClientID + ".turnPage(" + p + ",true);return false;\"><div data-page='"+ p + "' class='PagerPage' style=\"width:10px;\"></div></a>";
                    }
                    // Add start link
                    ...
                    bottomDiv += "<div style=\"clear:both;height:0;line-height:0;\"></div></div></div></div>";
                }
    ...
    {
        // Add the envelope
        e.Item.Controls.AddAt(0, new LiteralControl("<div data-page='" + index + "'  id=\"" + ClientID + "_" + index + "_content\" class='ContentPage' style=\"" + StyleOptions + "position:absolute;\" >"));
