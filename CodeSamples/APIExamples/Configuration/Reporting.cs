...
namespace APIExamples
{
    ...
    internal class Reporting
    {
        ...
        private class ReportCategories
        {
            ...
            {
                // Prepares a condition for loading all report categories whose code name starts with 'New'
                string where = "CategoryCodeName LIKE N'New%'";

                // Gets the report categories that fulfill the condition
                InfoDataSet<ReportCategoryInfo> categories = ReportCategoryInfoProvider.GetCategories(where, null);
        ...
        private class Reports
        {
            ...
            private void GetAndBulkUpdateReports()
            {
                ...
                // Prepares a condition for loading all reports within the specified category
                string where = "ReportCategoryID = " + category.CategoryID;

                // Gets all reports that fulfill the condition
                InfoDataSet<ReportInfo> reports = ReportInfoProvider.GetReports(where);
        ...
        private class ReportComponents
        {
            ...
            private void GetAndBulkUpdateReportComponents()
            {
                ...
                // Gets all graphs graphs defined for the specified report
                InfoDataSet<ReportGraphInfo> graphs = ReportGraphInfoProvider.GetGraphs(report.ReportID);
